...
export type AuditResponseFileRef = { path: string; fileName: string; mimeType: string; sizeBytes: number }
...
/**
 * Public Audit Response Link üzerinden dosyaları doğrudan Supabase Storage'a yükler
 * (Vercel gövde sınırını by-pass eder) — token ile kimliklendirilir, oturum gerekmez.
 */
export async function uploadAuditResponseFilesDirect(
  token: string,
  files: File[]
): Promise<AuditResponseFileRef[]> {
  if (files.length === 0) return []

  const res = await fetch(`/api/audit-response/${token}/files/upload-url`, {
    method: "POST",
  ...
  const results: AuditResponseFileRef[] = []
