...
export type AuditPlanDocumentRef = { path: string; fileName: string; mimeType: string; sizeBytes: number }
...
/** Denetim kaydı dosyalarını doğrudan Supabase Storage'a yükler (Vercel gövde sınırını by-pass eder). */
export async function uploadAuditPlanDocumentsDirect(
  entryId: string | number,
  files: File[]
): Promise<AuditPlanDocumentRef[]> {
  if (files.length === 0) return []

  const res = await fetch(`/api/audit-plan/${entryId}/documents/upload-url`, {
    method: "POST",
  ...
  const results: AuditPlanDocumentRef[] = []
