...
const PRISMA_CLIENT_CACHE_REVISION = 19 // bumped: AuditPlanEntry.auditType/auditingBodyTypeId, AuditCategoryType.scopes, AuditingBodyType, AuditPlanRevision(File)
...
function prismaHasExpectedDelegates(client: PrismaClient): boolean {
  const c = client as unknown as {
    ...
    manualChunk?: { deleteMany?: unknown }
    departmentPermission?: { findUnique?: unknown }
  }
  return (
    ...
    typeof c.manualChunk?.deleteMany === "function" &&
    typeof c.departmentPermission?.findUnique === "function"
  )
}
