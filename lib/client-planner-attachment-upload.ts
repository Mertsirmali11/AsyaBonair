...
export type PlannerAttachmentRef = { path: string; fileName: string; mimeType: string; sizeBytes: number }
...
/** Planner task eklerini doğrudan Supabase Storage'a yükler (Vercel gövde sınırını by-pass eder). */
export async function uploadPlannerTaskAttachmentsDirect(
  taskId: number | string,
  files: File[]
): Promise<PlannerAttachmentRef[]> {
  if (files.length === 0) return []

  const res = await fetch(`/api/planner/tasks/${taskId}/attachments/upload-url`, {
    method: "POST",
  ...
  const results: PlannerAttachmentRef[] = []
